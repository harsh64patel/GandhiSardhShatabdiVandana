    ...
    for page_num in tqdm(range(start_page, end_page + 1), desc="Scraping blog pages"):
        ...
        try:
            ...
            for article in articles:
                # Find the article URL from the more-link
                more_link = article.find('a', class_='more-link')
                if not more_link:
                    continue
                
                article_url = more_link.get('href')
                
                # Find the first link in the article which contains the title
                first_link = article.find('a')
                if first_link and "ગાંધી સાર્ધ શતાબ્દી વંદના" in first_link.text:
                    # Extract article number from the link text
                    match = re.search(r'ગાંધી સાર્ધ શતાબ્દી વંદના\s*[-–—]\s*(\d+)', first_link.text)
                    if match:
                        ...
                        logger.debug(f"Added article {article_num}: {article_url}")
